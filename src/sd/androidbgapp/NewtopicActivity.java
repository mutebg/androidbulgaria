...
public class NewtopicActivity extends Activity implements OnClickListener {
	...
	public String URL = "andapp/post.php?";
	...
	public void getData() {
		...
		TextView subjet 	= (TextView) findViewById(R.id.reply_subject);
		TextView message 	= (TextView) findViewById(R.id.reply_message);
		
		SharedPreferences settings = getSharedPreferences(PREFS_NAME, MODE_PRIVATE);
		URL += "&form_sent=1";
		URL += "&fid=" + forumID;
		URL += "&req_subject=" + URLEncoder.encode( subjet.getText().toString() );
		...
		//URL_PARAMS.put("fid", 		forumID + "" );
		//URL_PARAMS.put("form_sent", "1");
		//URL_PARAMS.put("req_subject", subjet.getText().toString() );
		...
		Thread t = new Thread() {
			...
			public void run() {
				try {
					...
					new AlertDialog.Builder(NewtopicActivity.this)
						.setTitle( "Грешка" )
